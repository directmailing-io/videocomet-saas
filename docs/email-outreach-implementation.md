...
- `src/lib/email/render.ts` — HTML-Renderer: table-basiertes 600px-Layout, Inline-Styles, substitute.ts für Platzhalter. **Freie Komposition (Update 2026-07-23):** Der Body (TipTap-bodyJson) ist die einzige Leinwand; GIF-Block und CTA-Button sind OPTIONALE, frei platzierbare Blöcke im Body (Custom-Nodes) — kein erzwungenes Layout, Text unter dem GIF möglich, Klartext-Links inline. Kein Block ⇒ nichts gerendert (blanke Mail möglich). PFLICHT bleibt (rechtlich, nicht abwählbar): dezente kleine graue „Abmelden"-Zeile (`{APP_URL}/abmelden/{token}`) + Impressum klein darunter am Mail-Ende. CTA-/GIF-Links laufen über `{APP_URL}/api/email/r/{token}` (Click-Redirect). Plain-Text-Ableitung.
