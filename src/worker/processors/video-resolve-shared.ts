...
// TODO Paket B: dieser Import existiert noch nicht — Paket B liefert
//   trackBunnyAsset / addBunnyAssetRef in @/lib/db/queries/bunny-assets.
//   Solange casten wir das Modul as any und tolerieren das Modul-fehlt-
//   Szenario im Runtime via try/catch.
type TrackBunnyAssetFn = (input: {
  userId: string;
  kind: "stream" | "storage";
  bunnyId: string;
  cdnUrl: string;
...
type AddBunnyAssetRefFn = (
  assetId: string,
  ownerType: "lead" | "run" | "media_item" | "campaign_webcam",
  ownerId: string,
) => Promise<void>;

async function loadBunnyAssetHelpers(): Promise<{
  trackBunnyAsset: TrackBunnyAssetFn | null;
  addBunnyAssetRef: AddBunnyAssetRefFn | null;
}> {
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mod = (await import("@/lib/db/queries/bunny-assets" as any)) as any;
    return {
      trackBunnyAsset: (mod.trackBunnyAsset ?? null) as TrackBunnyAssetFn | null,
      addBunnyAssetRef: (mod.addBunnyAssetRef ?? null) as AddBunnyAssetRefFn | null,
    };
  } catch {
    // Paket B noch nicht da → still skip, kein Tracking. Pipeline läuft trotzdem.
    return { trackBunnyAsset: null, addBunnyAssetRef: null };
  }
}
...
export interface VideoResolveSharedOutput {
  ...
  /** Bunny-Asset-Row-ID (für späteres addBunnyAssetRef(lead)). Null wenn Paket B fehlt. */
...
async function resolveExistingStream(
  ...
  helpers: { trackBunnyAsset: TrackBunnyAssetFn | null; addBunnyAssetRef: AddBunnyAssetRefFn | null },
): Promise<VideoResolveSharedOutput> {
  ...
  let bunnyAssetId: string | null = null;
  if (helpers.trackBunnyAsset && helpers.addBunnyAssetRef) {
    try {
      const asset = await helpers.trackBunnyAsset({
        userId: input.userId,
        kind: "stream",
        bunnyId: parsed.guid,
        cdnUrl: hlsUrl,
      ...
      bunnyAssetId = asset.id;
      await helpers.addBunnyAssetRef(asset.id, "run", input.runId);
    } catch (err) {
      console.warn(
        `[video-resolve-shared] bunny-asset track failed (stream): ${(err as Error).message}`,
      );
    }
  }
...
async function resolveByUpload(
  ...
  try {
    ...
    // Bunny-Meta für mp4Url / width / height (best-effort post-upload).
    ...
    let mp4Url: string | null = null;
    try {
      const meta = await getVideo(uploaded.videoId);
      const w = Number(meta.width ?? 0);
      const h = Number(meta.height ?? 0);
      if (w > 0 && h > 0) {
        width = w;
      ...
      console.warn(
        `[video-resolve-shared] post-upload meta failed: ${(err as Error).message}`,
      );
    }
    ...
    if (helpers.trackBunnyAsset && helpers.addBunnyAssetRef) {
      try {
        ...
        console.warn(
          `[video-resolve-shared] bunny-asset track failed (upload): ${(err as Error).message}`,
        );
      }
    }
...
export async function runVideoResolveShared(
  ...
  const helpers = await loadBunnyAssetHelpers();
  const t0 = Date.now();
  ...
  if (parsed) {
    out = await resolveExistingStream(input, parsed, helpers);
  } else {
    out = await resolveByUpload(input, helpers);
  }
  await insertPipelineEvent({
    runId: input.runId,
    leadId: null,
