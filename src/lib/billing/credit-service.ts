...
/**
 * Charge fuer einen E-Mail-Blast-Start (1 Credit = 10 Mails, aufgerundet).
 * MUSS innerhalb der Start-Transaktion laufen (tx-Parameter), damit bei
 * Insufficient die gesamte Materialisierung zurueckrollt.
 */
export async function chargeForEmailBlast(
  tx: Parameters<Parameters<typeof db.transaction>[0]>[0],
  input: { userId: string; blastId: string; amount: number },
): Promise<void> {
  ...
  const currentBalance =
    (row as { credit_balance: number } | undefined)?.credit_balance ?? 0;
  ...
  await tx.insert(creditTransactions).values({
    ...
    delta: -input.amount,
    balanceAfter: newBalance,
    reason: `email_blast:${input.blastId}`,
  });
  await tx
    .update(users)
    .set({ creditBalance: newBalance, updatedAt: new Date() })
...
/**
 * Refund fuer einen Blast unter einem festen Reason-Key.
 *
 * Idempotenz: refundet nur die Differenz zwischen `targetTotal` (kumulativ
 * erstattbare Credits fuer diesen Key) und der Summe bereits gebuchter
 * email_refund-Rows mit identischem Reason. Caller lockt vorher die
 * Blast-Row (FOR UPDATE), damit parallele Reconciles serialisiert sind.
 */
export async function refundEmailCreditsUpTo(
  tx: Parameters<Parameters<typeof db.transaction>[0]>[0],
  input: { userId: string; reasonKey: string; targetTotal: number },
): Promise<number> {
  if (input.targetTotal < 1) return 0;
  const [refunded] = await tx.execute<{ total: string | null }>(
    sql`SELECT COALESCE(SUM(delta), 0)::text AS total FROM credit_transactions
        WHERE user_id = ${input.userId} AND kind = 'email_refund'
          AND reason = ${input.reasonKey}`,
  );
  const already = Number(
    (refunded as { total: string | null } | undefined)?.total ?? 0,
  );
  const delta = input.targetTotal - already;
  if (delta <= 0) return 0;
