...
export function renderSlugTemplate(
  ...
  // {key} ersetzen — Whitespace im Key wird toleriert.
  const replaced = tpl.replace(/\{\s*([a-zA-Z0-9_]+)\s*\}/g, (_, key) =>
    lookupField(key, data),
  );
...
export async function generateSlug(opts: GenerateSlugOptions): Promise<string> {
  ...
  let base = renderSlugTemplate(opts.template ?? DEFAULT_SLUG_TEMPLATE, opts.leadData);
